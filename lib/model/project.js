...
exports.Project = function(config, redis) {
    ...
    this.findAction = function(proj,
                            triggerEvent, triggerRepo,
                            triggerBranch, done) {

        var hooksKey = 'projects:' + proj + ':hooks:' +
                                triggerRepo + ':events:' + triggerEvent;
        ...
        async.waterfall([
            ...
            function matchAction(actions, callback) {
                async.detectSeries(actions, function(actionMap, cb) {
                    ...
                    var reg = new RegExp(
                            '^' + actionMap.branch.replace('*', '.*') + '$');
                    cb(
                        actionMap.branch === '*' ||
                        actionMap.branch === triggerBranch ||
                        reg.test(triggerBranch)
                    );
                }, function(result) {
                    if (result) {
            ...
            hookGithub: function(callback) {
                async.forEach(Object.keys(hookIds), function(hookKey, cb2) {
                    var     parts = hookKey.split(':::')
                        ...
                        if (aevent === 'push' ||
                            aevent === 'pull_request') {
                            github.addHook(token, repo, arepo, aevent,
                                function(err, data) {
                                    if (err) cb2(err);
                                    else {
                                        hookIds[hookKey] = {
    ...
    this.cleanHooks = function(token, username, repo, done) {
        ...
        async.auto({
            ...
            cleanBuilds: function(callback) {
                self.builds.clean(repo, callback);
            }
    ...
    this.trigger = function(data, done)  {
        var self = this;
        async.waterfall([
            ...
            function getAuthUser(action, done) {
                ...
                self.getAuthUser(data.project, function(err, result) {
                    if (err) done(err);
                    else {
            ...
            function triggerAction(authUser, action, done) {
                self.builds.create(authUser, data, action, done);
            }
        ], done);
    },
    ...
    this.sync = function(token, username, repo, done) {
        ...
        async.auto({
            ...
            saveToken: ["clean", function cacheToken(callback) {
