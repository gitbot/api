...
var Builds = module.exports = function(config, redis)  {
    ...
    this.create = function(authUser, trigger, action, done) {
        ...
        async.auto({
            ...
            save: ["make", function(callback, res) {
                ...
                data.data = JSON.stringify(res.make.data);
                redis.multi()
    ...
    this.clean = function(project, done) {
        ...
        async.waterfall([
            ...
            function getBuildList(callback) {
                redis.lrange(
                    'projects:' + project + ':builds',
                        0, -1, callback);
            },

            function getBuildLogKeys(buildIds, callback) {
                async.map(buildIds, function(buildId, cb2) {
            ...
            function removeAllKeys(logKeyList, callback) {
                for (var index in logKeyList) {
                    keys.concat(logKeyList[index]);
                }
